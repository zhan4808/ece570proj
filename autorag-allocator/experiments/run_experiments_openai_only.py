#!/usr/bin/env python3
"""Run experiments using ONLY OpenAI models (faster, no rate limits)."""
import sys
import os
import json
import time
...
def run_baseline(dataset_name: str, dataset, corpus):
    """Run uniform baseline using GPT-4o-mini."""
...
def run_adaptive_allocation(dataset_name: str, dataset, corpus):
    """Run adaptive allocation system using only OpenAI models."""
    print(f"\n=== Running Adaptive Allocation (OpenAI Only): {dataset_name} ===")
    
    # Define model banks - ONLY OpenAI models
    retrievers = [
    ...
    generators = [
        GPT4oMiniGenerator()  # Only one generator for now
    ]
...
def main():
    ...
    print("=" * 60)
    print("AutoRAG-Allocator Experiment Runner (OpenAI Only)")
    print("=" * 60)
    ...
    results_file = results_dir / "full_results_openai_only.json"
